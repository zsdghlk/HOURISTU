...
function coerceParsed(p:any){
  if(!p||typeof p!=="object") return {lawTitle:"", meta:{}, articles:[] as any[]};
  if(!Array.isArray((p as any).articles)) (p as any).articles = [];
  return p as any;
}


// ---- minimal fetch helper (inline) ----
async function getXml(lawId: string): Promise<string> {
...
const q = (rawQ ?? "").trim();
  ...
  const xml = await getXml(lawId);
  const _parsed = await parseLawXml(xml);
  const parsed = coerceParsed(_parsed);

console.log("Server parsed.articles len:", Array.isArray(parsed?.articles) ? parsed.articles.length : -1);
const fallback = ROKUPO.find((x) => x.lawId === lawId)?.title;
  const pageTitle = parsed.lawTitle || fallback || "法令";
  let arts: Art[] = Array.isArray(parsed?.articles) ? parsed.articles : [];
  if (q) {
    arts = arts.filter(a => (a.number ?? "").includes(q) || a.paragraphs?.some(p => p.text?.includes(q)));
  }
  if (q) {
    arts = arts.filter(a => (a.number ?? "").includes(q) || a.paragraphs?.some(p => p.text?.includes(q)));
  }
console.log("Server arts init len:", Array.isArray(arts)?arts.length:-1);
  console.log("Server coerce check:", Array.isArray(arts) ? arts.length : -1);
...
arts = q
    ...
    : parsed.articles;
  console.log("DBG after filter:", Array.isArray(arts)? arts.length : -1);

  if (!Array.isArray(arts)) arts = [];

if (!Array.isArray(arts)) arts = [];
if (!Array.isArray(arts) || arts.length === 0) {
    return <div>条文が見つかりませんでした。</div>;
  }
// ====== 連続する prefix（附則ラベル）ごとにグループ化 ======
  ...
  {a.paragraphs.map((p, i) => {
    console.log("DBG para", { num: p?.num, head: (p?.text || "").slice(0, 32) });
    return (
      <p key={`-p`}>
        {formatParagraphNum(p.num, i)}{q ? highlight(p.text, q) : p.text}
      </p>
    );
  })}
</div>
