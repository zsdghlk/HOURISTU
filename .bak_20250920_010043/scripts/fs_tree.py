...
def should_ignore(name: str, relpath: str, patterns, extra_ignores, show_hidden):
    ...
    for p in extra_ignores:
        if fnmatch.fnmatch(name, p) or fnmatch.fnmatch(relpath, p):
            return True
    # .gitignore (シンプル対応: ファイル名 or 相対パスにグロブ)
    for p in patterns:
        # 先頭のスラッシュはプロジェクトルート基準
...
def to_text(root: Path, items):
    ...
    for kind, path, pre, truncated_flag in items:
        ...
        if truncated_flag:
            # ディレクトリ内の表示件数制限に到達
            base_pre = "    " if pre and pre[-1] else "│   "
            lines.append("".join(("".join("    " if p else "│   " for p in pre)) + "… (truncated)"))
    return "\n".join(lines)
...
def main():
    ...
    if args.json:
        ...
        header = "." if args.relative else root.name + "/"
        print(header)
        # 再構成してテキスト出力
        print(to_text(root, items).split("\n", 1)[1] if "/" in header else to_text(root, items))
